...
def unrotate(mouse_kps, mouse_rotation, mouse_center):
    ...
    mouse_kps = mouse_kps.reshape((-1, 10, 2))
    ...
    return mouse_keypoints.reshape(-1, 20)

def transform_svd_to_keypoints(data, svd_computer, mean, stack_agents = False,
                            stack_axis = 1, num_components = 7):

    svd_components = [data[:,i,4:] for i in range(3)]
    mouse_centers = [data[:,i,:2] for i in range(3)]
    mouse_rotations = [data[:,i,2:4] for i in range(3)]
